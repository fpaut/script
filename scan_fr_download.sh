...
extract_chapter_list()
{
    name="$1"
    html_content="$2"
    echo -e "$html_content" | grep --color=never -i "manga/$name/" | grep --color=never href | tac | while read url
    do
        chapt=${url##*$name/}
        echo ${chapt%\"*}
    done
    nb_ch=$(echo -e "$html_content" | grep --color=never -i "manga/$name/" | grep --color=never href | wc -l)
...
get_chapter()
{
    ...
    do
        ...
        logscreen_only $page_count/$nb_page
        logfile_only $file
