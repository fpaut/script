pattern=$1
CMD="adb shell su -c find /data/data -iname \"*$pattern*\" | grep -v \"POUBELLE\""
echo $CMD
FOLDER=$(eval $CMD)
...
if [[ "$FOLDER" != "" && "$FOLDER" != "." ]]; then
    ...
    if [ "$REPLY" == "y" ] || [ "$REPLY" == "Y" ]; then
        CMD="adb shell su -c mkdir /data/data/POUBELLE"
        echo $CMD; $CMD
        CMD="adb shell su -c mv  $FOLDER_TO_MV /data/data/POUBELLE/$FOLDER"
...
if [ "$REPLY" == "y" ] || [ "$REPLY" == "Y" ]; then
    CMD="adb shell su -c find /data/data -iname *POUBELLE*"
    echo $CMD; $CMD
    CMD="adb shell su -c ls -l /data/data/POUBELLE"
    echo $CMD; $CMD
fi
